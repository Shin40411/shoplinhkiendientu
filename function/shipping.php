...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($action == "add"){
    $sql_add_vanchuyen = mysqli_query($con, 
    ...
    $sql_update_vanchuyen = mysqli_query($con,
  ...
  if ($count > 0) {
    ...
    if (isset($row_getvanchuyen)) {
      ...
      if ($row_getvanchuyen['fullname'] == '' && $row_getvanchuyen['phone_number'] == '') {
        $shippinghtml .= '                 <div id="submitbutt" >  <button type="button" style="width:25%;float:left" onclick="updateShipping(\'add\')" id="updatevanchuyen" class="btn btn-success">Thêm thông tin</button> <button type="button" id="paystep" onclick="nextpaystep()" style="width:25%;float:right" class="btn btn-success">Bước kế tiếp<button></button> </div>';
      } else {
        $shippinghtml .= '                 <div id="submitbutt">  <button type="button" style="width:25%;float:left" onclick="updateShipping(\'update\')" id="updatevanchuyen" class="btn btn-primary">Cập nhật</button> <button type="button" id="paystep" onclick="nextpaystep()" style="width:25%;float:right" class="btn btn-success">Bước kế tiếp<button></button></div>';
      }
      $shippinghtml .= '                 </form>';
      $shippinghtml .= '               </div>';
      $shippinghtml .= '             </div>';
